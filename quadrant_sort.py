...
def quadrant_sort_function():
    ...
    # 도시를 사분면으로 나눈 후, 원점과의 거리를 기준으로 정렬
    sorted_cities = sorted(cities, key=lambda city: (quadrant(city[:2]), distance(city[:2], [0,0])))

    # 도시를 index만 저장
    sorted_cities= [row[2] for row in sorted_cities]
